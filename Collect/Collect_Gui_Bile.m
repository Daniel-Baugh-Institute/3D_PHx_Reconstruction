function Collect_Gui_Bile(Path_List,In_Path)
...
for i=1:numel(Path_List)
    ...
    if exist(fullfile(Path,'Bile_Data_Link_First_Order'),'file') ==2
        ...
        %2nd Order
        Data = csvread(fullfile(Path,'Bile_Data_Link_Second_Order'));
        Data = Data(:,Cols2);
        T = time*ones(size(Data,1),1);
...
xlswrite(fullfile(In_Path,'Bile_Data_Labels'),headers_1,'Link_1st_Order');
xlswrite(fullfile(In_Path,'Bile_Data_Labels'),headers_2,'Link_2nd_Order');
csvwrite(fullfile(In_Path,'Bile_1st_Order'),Data1st);
csvwrite(fullfile(In_Path,'Bile_2nd_Order'),Data2nd);
end
